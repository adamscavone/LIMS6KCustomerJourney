...
const Receiving1 = () => {
  ...
  const [showReviewModal, setShowReviewModal] = useState(null); // manifestId when showing review modal
  ...
  const handleReceiveManifest = (manifestId) => {
    ...
    console.log('Found manifest to receive:', manifestToReceive);
    ...
    if (manifestToReceive) {
      ...
      console.log('Creating received manifest:', receivedManifest);
      
      // Add to received manifests
      setReceivedManifests(prev => {
        const newReceivedManifests = [...prev, receivedManifest];
        console.log('New received manifests:', newReceivedManifests);
        return newReceivedManifests;
      });
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      {/* Review & Submit Modal */}
      {showReviewModal && (() => {
        const manifest = manifests.find(m => m.manifestId === showReviewModal);
        if (!manifest) return null;
        
        const manifestDataForReview = manifestData[manifest.manifestId] || {};
        const samplesWithData = manifest.samples.map((sample, idx) => ({
          ...sample,
          ...(manifestDataForReview.samples?.[idx] || {})
        }));
            ...
            <div className="flex items-center justify-center min-h-screen px-4">
              <div className="bg-white rounded-lg max-w-4xl w-full p-6 my-8">
                ...
                <div className="mb-6">
                  ...
                  <div className="mt-4 flex items-center justify-center space-x-4">
                    <div className="flex items-center">
                      ...
                      <span className="ml-2 text-sm text-gray-600">Select Manifest</span>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="ml-2 text-sm text-gray-600">Fill Details</span>
                    </div>
                    <div className="w-16 h-0.5 bg-gray-300"></div>
                    <div className="flex items-center">
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-6">
                  <h3 className="text-lg font-medium text-gray-900 mb-3">Manifest Summary</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-600">Client</p>
                      <p className="font-medium text-gray-900">{manifest.customerFacility}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Manifest Number</p>
                      <p className="font-medium text-gray-900">{manifest.manifestId}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">CC Order ID</p>
                    ...
                    {rushCount > 0 && (
                      <div className="flex items-center">
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">
                          ...
                          {rushCount} Rush {rushCount === 1 ? 'Sample' : 'Samples'}
                        </span>
                      </div>
                    )}
                    {terpenesCount > 0 && (
                      <div className="flex items-center">
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">
                          {terpenesCount} Terpenes Testing
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div className="mb-6">
                  <h3 className="text-lg font-medium text-gray-900 mb-3">Sample Details</h3>
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {samplesWithData.map((sample, idx) => (
                      <div key={idx} className="border border-gray-200 rounded-lg p-3 bg-white">
                        <div className="flex items-start justify-between">
                          <div>
                            <p className="font-medium text-gray-900">
                              {sample.ccId || `Sample ${idx + 1}`} - {sample.strain || sample.itemName}
                            </p>
                            <p className="text-sm text-gray-600">
                              Type: {sample.sampleType || 'Not specified'} | 
                              Category: {sample.testCategory || 'Not specified'}
                            </p>
                          </div>
                ...
                <div className="flex justify-between">
                  ...
                  <button
                    onClick={() => {
                      handleReceiveManifest(manifest.manifestId);
                ...
                {manifests.map(manifest => {
                  ...
                  return (
                    ...
                    <div className="mt-4 flex justify-end space-x-3">
                      ...
                      <button
                        onClick={() => setShowReviewModal(manifest.manifestId)}
